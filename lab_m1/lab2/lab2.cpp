...
void Lab2::CreateCircle() {
    {
        ...
        for (int i = 0; i < n; i++) {
            if (i % 2 == 0)
                vertices.push_back(VertexFormat(glm::vec3(r * cos(alpha), r * sin(alpha), 0), glm::vec3(1, 1, 0)));
            else
                vertices.push_back(VertexFormat(glm::vec3(r * cos(alpha), r * sin(alpha), 0), glm::vec3(1, 0, 0)));
            alpha += (double)2 * M_PI / n;
        }
...
void Lab2::Update(float deltaTimeSeconds)
{
    ...
    RenderMesh(meshes["cube_B"], shaders["VertexColor"], glm::vec3(0.5f, 1.5f, -1), glm::vec3(0.25f));

    // TODO(student): Draw the tetrahedron
    RenderMesh(meshes["tetra"], shaders["VertexColor"], glm::vec3(-1.2f, 2, -2), glm::vec3(0.5f));

    // TODO(student): Draw the square
    RenderMesh(meshes["square"], shaders["VertexColor"], glm::vec3(1.5f, 1.2f, 0), glm::vec3(0.75f));

    // Draw the circle
    CreateCircle();
    RenderMesh(meshes["circle"], shaders["VertexColor"], glm::vec3(-3, 2, -2), glm::vec3(0.75f));
